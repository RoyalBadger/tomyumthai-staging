...
import { ITEM_MODIFIER_SQL, removeLabel, extraLabel } from './modifiers.js';
...
export async function loadPricingContext(promoCode) {
  const [items, sizes, variants, modifiers, proteins, extras, settings, promo] = await Promise.all([
    ...
    query('SELECT item_id, label, delta_cents FROM item_variants', []),
  ...
  for (const v of variants.rows) (variantsByItem[v.item_id] ??= []).push(v);
  const modifiersByItem = {};
  for (const m of modifiers.rows) (modifiersByItem[m.item_id] ??= []).push(m);
  return {
 ...
 *              items:[{id, size_label?, variant?, protein?, extras?:string[], spice_level?,
 *                      modifiers?:[{id, kind:'remove'|'extra'}], exclusions?, notes?, qty}]}}
 ...
 *           promo_code, lines:[{item_id,name,size_label,variant,protein,extras,spice_level,
...
export function priceCart(cart, ctx) {
  ...
  for (const raw of cart.items) {
    ...
    // per-dish "choose one" variant (e.g. Chicken/Pork/Vegetable crispy rolls,
    // Red/Green curry). Required when the dish defines any; may carry an upcharge.
    const variants = (ctx.variantsByItem || {})[item.id] || [];
    let variantLabel = null;
    if (variants.length > 0) {
      const v = variants.find(x => x.label === raw.variant);
      if (!v) throw new CartError(`Please make a selection for ${item.name}.`);
      base += v.delta_cents || 0;
      variantLabel = v.label;
    } else if (raw.variant) {
      throw new CartError(`${item.name} has no selection options.`);
    }
    ...
    // ingredient modifiers: remove (free → exclusions line) / extra (upcharge → add-ons)
    const removals = [];
    if (raw.modifiers?.length) {
      const allowed = (ctx.modifiersByItem || {})[item.id] || [];
      if (raw.modifiers.length > 20) throw new CartError('Too many changes on one item.');
      const seen = new Set();
      for (const mod of raw.modifiers) {
        const key = `${mod?.id}:${mod?.kind}`;
        if (seen.has(key)) throw new CartError('Duplicate modifier.');
        seen.add(key);
        const def = allowed.find(a => a.modifier_id === Number(mod?.id));
        if (!def) throw new CartError(`That change isn't available for ${item.name}.`);
        if (mod.kind === 'remove') {
          if (!def.can_remove) throw new CartError(`${item.name} can't be made without ${def.label}.`);
          removals.push(removeLabel(def));
        } else if (mod.kind === 'extra') {
          if (!def.can_extra) throw new CartError(`Extra ${def.label} isn't available for ${item.name}.`);
          base += def.extra_cents_override ?? def.extra_cents ?? 0;
          extraLabels.push(extraLabel(def));
        } else {
          throw new CartError('Unknown modifier type.');
        }
      }
      if (extraLabels.length > 8) throw new CartError('Too many add-ons on one item.');
    }
    ...
    const exclusionText = removals.concat(raw.exclusions ? [String(raw.exclusions)] : []).join(', ');
    const exclusions = exclusionText ? exclusionText.slice(0, MAX_NOTE).toUpperCase() : null;
    const notes = raw.notes ? String(raw.notes).slice(0, MAX_NOTE) : null;

    subtotal += base * qty;
    lines.push({
      item_id: item.id, name: item.name, size_label: sizeLabel, variant: variantLabel, protein: proteinLabel,
